...
export async function getDoctors() {
    try {
        ...
        // Return the array of doctors (handles different backend response shapes)
        return data.doctors ? data.doctors : data;
    } catch (error) {
...
export async function deleteDoctor(id, token) {
    try {
        const response = await fetch(`${API_BASE_URL}/admin/doctor/${id}/${token}`, {
            method: 'DELETE'
        });
        const data = await response.json();
        return {
...
export async function saveDoctor(doctor, token) {
    try {
        const response = await fetch(`${API_BASE_URL}/admin/doctor/${token}`, {
            method: 'POST',
            headers: {
...
export async function filterDoctors(name, time, specialty) {
    try {
        // Uses the exact URL format required by your Spring Boot backend
        ...
        alert("An error occurred while trying to filter the doctors.");
        return [];
    }
}
