...
console.log('masuk');
// --- REMOVED DOMContentLoaded WRAPPER: Modules run safely on their own! ---

// 1. Load initial doctor cards immediately
loadDoctorCards();
...
const filterTimeElement = document.getElementById("timeFilter");
const filterSpecialtyElement = document.getElementById("specialtyFilter");

// Attach the event listeners so they actually trigger when you type
if (searchBarElement) searchBarElement.addEventListener("input", filterDoctorsOnChange);
if (filterTimeElement) filterTimeElement.addEventListener("change", filterDoctorsOnChange);
if (filterSpecialtyElement) filterSpecialtyElement.addEventListener("change", filterDoctorsOnChange);
...
async function filterDoctorsOnChange() {
    ...
    const filterTimeVal = document.getElementById("timeFilter").value;
    const filterSpecialtyVal = document.getElementById("specialtyFilter").value;

    // FIX: Send the literal string "null" to match the Spring Boot backend expectations!
...
window.adminAddDoctor = async function (event) {
    ...
    // FIX: These IDs now perfectly match your modals.js file!
